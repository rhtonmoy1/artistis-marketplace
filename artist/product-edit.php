...
if(isset($_POST['form1'])) {
    ...
    if($valid == 1) {
    	...
    	if( isset($_FILES['photo']["name"]) && isset($_FILES['photo']["tmp_name"]) )
        {
        	...
        	$statement = $pdo->prepare("SHOW TABLE STATUS LIKE 'tbl_art_photo'");
			$statement->execute();
            ...
            if(isset($final_name1)) {
		        ...
		        {
		        	$statement = $pdo->prepare("INSERT INTO tbl_art_photo (photo,p_id) VALUES (?,?)");
        ...
        if($path == '') {
        	$statement = $pdo->prepare("UPDATE tbl_art SET 
        							p_name=?, 
        							p_old_price=?, 
        							p_current_price=?, 
        							p_qty=?,
        ...
        if(isset($_POST['size'])) {
        	...
        	$statement = $pdo->prepare("DELETE FROM tbl_art_size WHERE p_id=?");
        	$statement->execute(array($_REQUEST['id']));

			foreach($_POST['size'] as $value) {
				$statement = $pdo->prepare("INSERT INTO tbl_art_size (size_id,p_id) VALUES (?,?)");
		...
		if(isset($_POST['color'])) {
			...
			$statement = $pdo->prepare("DELETE FROM tbl_art_color WHERE p_id=?");
        	$statement->execute(array($_REQUEST['id']));

			foreach($_POST['color'] as $value) {
				$statement = $pdo->prepare("INSERT INTO tbl_art_color (color_id,p_id) VALUES (?,?)");
				$statement->execute(array($value,$_REQUEST['id']));
			}
		} else {
			$statement = $pdo->prepare("DELETE FROM tbl_art_color WHERE p_id=?");
...
if(!isset($_REQUEST['id'])) {
	...
	$statement = $pdo->prepare("SELECT * FROM tbl_art WHERE p_id=?");
	$statement->execute(array($_REQUEST['id']));
...
$statement = $pdo->prepare("SELECT * FROM tbl_art_size WHERE p_id=?");
$statement->execute(array($_REQUEST['id']));
$result = $statement->fetchAll(PDO::FETCH_ASSOC);							
foreach ($result as $row) {
	$size_id[] = $row['size_id'];
...
$statement = $pdo->prepare("SELECT * FROM tbl_art_color WHERE p_id=?");
$statement->execute(array($_REQUEST['id']));
$result = $statement->fetchAll(PDO::FETCH_ASSOC);							
foreach ($result as $row) {
						...
						<div class="form-group">
							...
							<div class="col-sm-4" style="padding-top:4px;">
								<table id="ProductTable" style="width:100%;">
			                        <tbody>
			                        	<?php
			                        	$statement = $pdo->prepare("SELECT * FROM tbl_art_photo WHERE p_id=?");
			                        	$statement->execute(array($_REQUEST['id']));
			                        	$result = $statement->fetchAll(PDO::FETCH_ASSOC);
			                        	foreach ($result as $row) {
			                        		?>
