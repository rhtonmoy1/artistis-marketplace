...
<?php	
	...
	if(isset($ecat_ids)) {
		...
		for($i=0;$i<count($ecat_ids);$i++) {
			$statement = $pdo->prepare("SELECT * FROM tbl_art WHERE ecat_id=?");
		...
		for($i=0;$i<count($p_ids);$i++) {
			...
			$statement = $pdo->prepare("DELETE FROM tbl_art WHERE p_id=?");
			$statement->execute(array($p_ids[$i]));

			// Delete from tbl_art_photo
			$statement = $pdo->prepare("DELETE FROM tbl_art_photo WHERE p_id=?");
			$statement->execute(array($p_ids[$i]));

			// Delete from tbl_art_size
			$statement = $pdo->prepare("DELETE FROM tbl_art_size WHERE p_id=?");
			$statement->execute(array($p_ids[$i]));

			// Delete from tbl_art_color
